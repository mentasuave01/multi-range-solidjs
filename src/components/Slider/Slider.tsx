...
export function Slider<T extends number | number[]>(props: SliderProps<T>) {
  ...
  const smallestValue = createMemo(() =>
    values().length == 1 ? 0 : Math.min(...values())
  );

  const biggestValue = createMemo(() => Math.max(...values()));

  const currentActiveIndex = createMemo(() =>
    thumbs.findIndex((thumb) => thumb.active)
  );
  ...
  const calcValueFromPortion = (portion: number, whole: number) =>
    (portion / whole) * diff() + min();
  ...
  const handleInput: JSX.EventHandlerUnion<HTMLInputElement, InputEvent> = (
    ...
    setThumbs(
      Number(currentTarget.dataset.index),
      "value",
        ...
        {(thumb, index) => (
          ...
          >
            <input
              ...
              onInput={handleInput}
            />
            <span class="value">{thumb.value}</span>
          </span>
        )}
      </For>
    </span>
